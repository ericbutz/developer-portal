...
You can check the status of a transfer at any time by [retrieving the transfer via the API.](https://docsv2.dwolla.com/#retrieve-a-transfer) When a bank transfer is unable to be completed, its status will be updated to `failed`. The response from the API when retrieving the transfer should contain a `"failure"` link that your application will follow to [retrieve the transfer failure reason](https://docsv2.dwolla.com/#retrieve-a-transfer-failure-reason) in the next step.
